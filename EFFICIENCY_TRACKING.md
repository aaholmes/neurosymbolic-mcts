...
**6a. Small-network Lc0 import (validation step).** Take a Maia-class Lc0 network (6×64 or 10×128 T-net) and reuse its body and policy head directly; replace its value head with a small adapter trained to predict the residual on top of NNUE-Q. Inherits Lc0's training data implicitly via the pretrained weights. The adapter learns to improve the evaluation in ways that aren't easy to capture by a Q-search over a shallow network — long-horizon and global-pattern judgments that benefit from a deeper feature stack and explicit policy guidance. Fine-tuning: freeze the network except the value head, add a small FC adapter on top of the value-head input, keep `k_logit` as a tunable scalar, train MSE against Stockfish-labeled positions. ~3–4 weeks (after the network-adapter layer is in place). Validates the network-import architecture before committing to BT4.

**6b. Import BT4 directly (strategic destination).** Once 6a is validated, swap the small network for Lc0's actual best mainline (BT4 or successor). Under CPU-resident MCTS + batched GPU inference, network size is no longer a constraint — standard libraries handle 768×15 at high batch sizes natively. Same network signal as vanilla Lc0+BT4 plus tactical resolution (NNUE-Q) plus CPU/GPU concurrency plus selective deepening. Realistic ceiling: **~+30 to +100 Elo over vanilla Lc0+BT4 on equivalent hardware** (puts us at top-5 worldwide, in striking distance of SF17). Three integration challenges: combination function (BT4's value already encodes material — naive addition double-counts; needs adapter fine-tuning OR use NNUE-Q only as extension trigger), BT4 loader (~2 weeks, protobuf format, GPL-contamination risk if reading Lc0's code), fine-tuning compute (a few GPU-days, Stockfish-labeled positions). ~6–8 weeks after 6a is validated. See DESIGN_DECISIONS §8 "Importing BT4 directly" for full detail.

The NNUE-Q-search change is the highest-ROI next step. It's small, delivers strength gains independent of any larger architectural decision, and validates the "NNUE as fast eval" hypothesis before committing to the bigger work. Lc0 network import is the natural follow-up — it depends on NNUE-Q being in place (the adapter is trained against `NNUE_q`, not against PeSTO's ΔM) and on the network-adapter layer being in place. The async pipeline can be staged in either before or after the network import; doing it before makes the network import simpler because there's already a queue to feed.

**Realistic full timeline to "compete with Lc0+BT4":** ~4-6 months for the full pipeline (NNUE-Q → retrain → adapter layer → async pipeline → small-network Lc0 import → BT4 import → iterative deepening + tuning). Each step delivers strength gains independently — none wasted if a later step doesn't pan out.
