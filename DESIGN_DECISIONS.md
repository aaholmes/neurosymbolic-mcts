...
**NNUE source: Akimbo, not Stockfish.** A Phase 3 spike on the originally-planned Stockfish `.nnue` integration revealed two blockers: noru only loads its own NORU-magic v2/v3 binary format (it cannot parse Stockfish `.nnue`), and modern Stockfish uses HalfKAv2_hm with 8 output buckets selected by piece count — incompatible with noru's single-bucket architecture even with a from-scratch parser. Pivot to Akimbo (https://github.com/jw1912/akimbo, MIT-licensed code and weights). Akimbo's NNUE is `(768 × 4_king_input_buckets → 1024)_per_perspective → concat → 1`, SCReLU activation, single output bucket — directly mappable to noru as `NnueConfig::new_static(3072, 1024, &[], Activation::SCReLU)` with the king-bucket lookup baked into the feature encoder. The file format is a raw `i16` struct dump (`mem::transmute` of `Network { feature_weights, feature_bias, output_weights, output_bias }`, QA=255, no header) — loading is roughly a memcpy. Akimbo at ~3470 CCRL Blitz is ~150-200 Elo below the Stockfish/Berserk/Lc0 cluster but ~+1000 Elo over PeSTO, far more than enough for our use case. The strategic upside: Akimbo is by jw1912, the author of `bullet` (the Rust+CUDA NNUE trainer used by most modern open-source NNUE engines). Adopting Akimbo's file format means automatic compatibility with bullet, so any future custom NNUE we train (specialized for KOTH, different data, different architecture) drops into the same loader without changes. Akimbo isn't the destination — it's the on-ramp to the bullet ecosystem.
